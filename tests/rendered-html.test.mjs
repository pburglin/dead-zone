...
test("uses deterministic three-region TV navigation and keeps branding out of the focus path", async () => {
  for (const feature of ['data-tv-region="left"', 'data-tv-region="right"', "data-tv-map", "data-tv-cell", "data-tv-x", "data-tv-y", "navigateGame", "navigateMenu", ".global-audio button", "focusActiveCell", "focusRegion", 'current.matches("[data-tv-map]")', 'e.key==="Enter"&&target?.matches("[data-tv-map]")', "__deadZonePendingKeys", "requestAnimationFrame(()=>document.querySelector<HTMLElement>(\"[data-active='true']\")?.focus())"]) assert.ok(source.includes(feature), `missing TV navigation feature ${feature}`);
